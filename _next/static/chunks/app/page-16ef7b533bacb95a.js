...
r = () => {
  ...
  // g_updateGameState 现在接收最终计算好的氧气值
  const g_updateGameState = (0, l.useCallback)(
    (currentGrid_g, finalOxygenLevelForThisTurn) => {
      ...
      let tiger_count_g = 0;
      let wood_count_g = 0;
      // activeHumanCount_forOxygen 将在 m_handleCellClick 中用于直接修改氧气

      currentGrid_g.flat().forEach((cell_g) => {
        if ("plant" === cell_g.content) plant_count_g++;
        if ("human" === cell_g.content) human_count_display++; // 只统计格子内容为human的，用于UI显示
    ...
    [], // 移除了 t.currentLevel，因为氧气计算在 m_handleCellClick 中
  );
  ...
  const m_handleCellClick = (row_idx, col_idx) => {
    ...
    let message_click = "";
    let shouldUpdateGridAndOxygen = true; // 是否需要调用 g_updateGameState
    let newOxygenLevel = t.oxygenLevel; // 获取当前的氧气值作为计算基础
    ...
    if (r_selectedItem) {
      if (
        ...
        if ("plant" === r_selectedItem) {
          if (clickedCell.isFertile || "ash" === clickedCell.content) {
            ...
            // 根据植物类型和土地状态直接增减氧气
            if (t.currentLevel === 3 && !clickedCell.isFertile && clickedCell.content === "plant") {
                 // ash 已经是肥沃的了，所以这个分支不会被贫瘠土地走到
                newOxygenLevel += 10; // 即使是第3关，灰烬上种植物也按10算
            } else if (t.currentLevel === 3 && !clickedCell.isFertile){ // 这个是针对非灰烬的贫瘠土地
                 newOxygenLevel += 1;
            } else {
                 newOxygenLevel += 10;
            }
      ...
      const oxygenContributionOfThisPlant = (plantInitialLevel === 3 && !plantInitialFertile) ? 1 : 10;
      ...
      if (i(row_idx, col_idx, gridCopy_click)) {
        ...
        newOxygenLevel -= oxygenContributionOfThisPlant;
    ...
    if (shouldUpdateGridAndOxygen) {
      ...
      let finalGrid = gridCopy_click; // 默认使用已经过上述操作修改的 gridCopy_click

      if (t.currentLevel >= 2) {
        // 为老虎吃人逻辑创建一个新的 grid 副本，以防修改影响氧气计算
        let gridForTigerCheck = gridCopy_click.map(row => row.map(cell => ({...cell})));
        let humansBeforeTigerEat = 0;
        gridForTigerCheck.flat().forEach(cell => {
            if (cell.content === 'human' || cell.owner === 'human') humansBeforeTigerEat++;
        });

        for (let r_tg_row = 0; r_tg_row < 5; r_tg_row++) {
          for (let c_tg_col = 0; c_tg_col < 4; c_tg_col++) {
            if ("tiger" === gridForTigerCheck[r_tg_row][c_tg_col].content) {
              ...
              for (let [dr_tg, dc_tg] of [
                ...
                if (h_row >= 0 && h_row < 5 && h_col >= 0 && h_col < 4) {
                  let targetCell_tg = gridForTigerCheck[h_row][h_col];
                  if ("human" === targetCell_tg.content && targetCell_tg.owner !== "human") {
                    gridForTigerCheck[h_row][h_col].content = "human-dead";
                    gridForTigerCheck[h_row][h_col].isDecaying = !0;
        ...
        if (tigerEats_flag) {
          finalGrid = gridForTigerCheck;
          // 如果老虎吃了人，重新计算实际耗氧人数，并调整 newOxygenLevel
          let activeHumansAfterTigerEat = 0;
          finalGrid.flat().forEach(cell => {
              if(cell.content === "human" || cell.owner === "human") activeHumansAfterTigerEat++;
          });
          // 之前 newOxygenLevel 是基于 humansBeforeTigerEat 计算的（间接通过 t.oxygenLevel - 5*newly_added_human）
          // 现在需要调整回正确的耗氧。
          // 这是一个复杂点，因为 newOxygenLevel 已经包含了其他操作的影响。
          // 最简单的是，如果老虎吃了人，则强制 g_updateGameState 重新计算氧气。
          newOxygenLevel = null; // 标记让 g_updateGameState 自己计算
        }
      }
      g_updateGameState(finalGrid, newOxygenLevel);
    }
  };
